      ...
      .then((body) => {
        ...
        if (isFirst) {
          ...
          setBigNews({
            ...
            ThumbnailUrl: body[0].ThumbnailUrl,
          });
        }
        setLoading(false);
        setIsFirst(false);
      ...
      {isDownMD ? (
        <Row justify="center" style={MainRowSmall}>
          <Row style={{ width: "100%", paddingRight: "5%" }}>
            <Image
              src={imageUrlBuilder(bigNews.ThumbnailUrl)}
              preview={{ src: imageUrlBuilder(bigNews.ImageUrl) }}
              height="100%"
              width="100%"
        ...
        <Row justify="start" style={MainRow} gutter={[12, 8]}>
          ...
          <Col flex="1 1 300px">
            <div
              id="scrollableDiv"
              style={{ height: 400, overflow: "auto", marginRight: 50 }}
            >
              <InfiniteScroll
                dataLength={data.length}
                next={loadMoreData}
